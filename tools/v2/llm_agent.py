...
from .order_packet_filters import sanitize_player_orders
...
def decide_orders(
    ...
    # Same packet cleanup as the human web UI (engine2 orderPacketFilters).
    start_inf = int((view.get("my_player_state") or {}).get("influence") or 0)
    if orders:
        before = len(orders)
        orders = sanitize_player_orders(start_inf, orders)
        if diagnostics is not None and len(orders) < before:
            diagnostics.append(
                f"dropped {before - len(orders)} order(s) (duplicate moves per force "
                f"and/or influence budget; parity with human queue)"
            )
