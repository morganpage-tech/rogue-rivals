...
- Rogue flavor system (see GDD §5.3 ? narrative overlay, purely text)
...
- Thread view shows all public events (per `RULES.md` §8 glossary)
...
| `event_type` | TEXT | from `SIMULATION_SCHEMA.md` §events |
...
- After every `take_action`, the engine advances to the next player and runs end-of-round resolution / match-end check as specified in `RULES.md` §2 and §5.
...
- `players[Pj].beads`: visible (Beads are public per `RULES.md` §8)
- `players[Pj].vp`: redact to ordinal rank only (per `RULES.md` §6.1)
...
- Fixed logical resolution: **640×360** (16:9), scaled to viewport with `ctx.scale`
...
Event types rendered (from `RULES.md` §8):
...
Per `RULES.md` §3.2, offers expire when the offerer's next turn begins. Server runs this automatically in the turn-start hook and emits `trade_expired` events.
...
### M1 ? Local hot-seat
- [ ] `packages/engine` bootstrapped with types from §6.1
...
### M2 ? Async server
- [ ] Fastify server with endpoints in §7.1
- [ ] WebSocket hub with messages in §7.2
- [ ] SQLite schema in §5 migrated
- [ ] Magic-link flow end-to-end (create ? invite ? join ? play)
- [ ] React client with `MatchMain` layout §8.2
- [ ] Canvas renders all 5 regions and 5 building types at target performance
- [ ] Thread renders every event type in §8.4 table
...
pnpm -F @rr/engine init
pnpm -F @rr/server  init
pnpm -F @rr/web     init

# Engine first
cd packages/engine
# Port tools/sim.py semantics to TS; start with types from §6.1
# Target: all 50 v0.7.3 logs replay green

# Verify determinism
pnpm -F @rr/engine test:replay ../../simulations/batch_v0.7.3.jsonl

# Then server
cd ../server
# Implement endpoints §7.1, plumb engine

# Then web
cd ../web
# React + Vite + Canvas layer, build MatchMain §8.2
```
...
- `/packages/engine/**` (new TS package)
- `/packages/shared/**`
- `/packages/server/**`
- `/packages/web/**`
- `/Dockerfile`
